...
def evalrank(img_embs, cap_embs, opt, fold5=False):
    if not fold5:
        # no cross-validation, full evaluation
        r, rt = i2t(img_embs, cap_embs, measure=opt.measure, return_ranks=True)
        ri, rti = t2i(img_embs, cap_embs,
...
def main(batches):
    ...
    for batch in batches:
        ...
        batch_pairs = batch['positive_pairs']

        num_images += batch_images.size(0)
        num_texts += batch_texts.size(0)

        images = batch_images[batch_pairs[:,0]]
        captions = batch_texts[batch_pairs[:,1]]

        image_batches.append(images.numpy())
        caption_batches.append(captions.numpy())


    images = np.concatenate(image_batches, axis=0)
    captions = np.concatenate(caption_batches, axis=0)
    
    opt = Option()
    evalrank(images, captions, opt)
...
if __name__ == '__main__':
    ...
    batches = pickle.load(open(sys.argv[1], 'rb'))
    main(batches)
